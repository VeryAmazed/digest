...
void WindowMin_dig_comp(digest::WindowMin<P, T> &dig1,
  ...
  // need to use this because I need to check, or at least get some indication,
  // of whether the two seg trees are the same
...
TEST_CASE("Digester Testing") {
  ...
  SECTION("Base Constructor Special Cases") {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        for (int l = 0; l < 16; l++) {
          ...
          for (int p = 0; p < 3; p++) {
            ...
            digest::ModMin<digest::BadCharPolicy::SKIPOVER> *dig =
                new digest::ModMin<digest::BadCharPolicy::SKIPOVER>(
                    test_strs[i], k, mod, congruence, pos, minimized_h);
            ModMin_constructor(*dig, test_strs[i], k, pos, minimized_h, mod,
                               congruence);
            delete dig;
          }
        }
    ...
    for (int j = 0; j < 8; j++) {
      ...
      for (int l = 0; l < 16; l++) {
        ...
        for (int p = 0; p < 3; p++) {
          ...
          ModMin_constructor_writeover(*dig, test_strs[4], k, pos, minimized_h,
                                       mod, congruence);
          delete dig;
        }
      }
    ...
    k = 0;
    digest::ModMin<digest::BadCharPolicy::SKIPOVER> *dig;
    CHECK_THROWS_AS(dig = new digest::ModMin<digest::BadCharPolicy::SKIPOVER>(
                        str, k, mod, congruence, pos, minimized_h),
  ...
  SECTION("Testing append_seq()") {
    ...
    for (int j = 0; j < 8; j++) {
      for (int l = 15; l < 91; l += 15) {
        ...
        digest::ModMin<digest::BadCharPolicy::WRITEOVER> *dig =
            new digest::ModMin<digest::BadCharPolicy::WRITEOVER>(
                str1, ks[j], 1e9 + 7, 0, 0, digest::MinimizedHashType::FORWARD);
    ...
    for (int i = 0; i < 7; i += 2) {
      for (int j = 0; j < 8; j++) {
        for (int l = 0; l < 3; l++) {
          ...
          ModMin_roll_minimizer(*dig, test_strs[i], ks[j],
                                static_cast<digest::MinimizedHashType>(l),
                                prime);
          delete dig;
        }
      ...
      for (int j = 0; j < 8; j++) {
        for (int l = 15; l < 91; l += 15) {
          ...
          digest::ModMin<digest::BadCharPolicy::SKIPOVER> *dig2 =
              new digest::ModMin<digest::BadCharPolicy::SKIPOVER>(*dig1);
...
#define TEST_ROLL_0(p, m, k)                                                   \
  ...
  /*
          the below also inadverntently tests how append_seq (only the case that
     there are 2 sequences involved total) works with roll_minimizer for
     WindowMin. In theory this shouldn't be needed and also can't be considered
     "thorough", but it is extra assurance.
  */
  SECTION("Testing Copy Constructor") {
    for (int i = 0; i < 7; i += 2) {
      for (int j = 0; j < 8; j++) {
...
TEST_CASE("Syncmer Testing") {
  // Syncmer and WindowMinimizers have all the same class members so I can just
  // use the WindowMin tests for Constructor and be ok
  SECTION("Constructor Testing") {
    ...
    // digest::Syncmer<data_structure::data_structure::SegmentTree<0>>(str, k,
    // 0, pos, minimized_h)), digest::BadWindowSizeException);
