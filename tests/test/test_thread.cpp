...
void test_thread_wind(unsigned thread_count, std::string str, unsigned k,
  ...
  std::vector<uint32_t> single_thread;
  std::vector<std::vector<uint32_t>> vec;
  digest::WindowMin<digest::BadCharPolicy::SKIPOVER, digest::ds::Adaptive> dig(
      ...
      for (int i = 0; i < 4; i += 2) {
        for (int j = 4; j <= 64; j += 4) {
          ...
          for (int l = 0; l <= 96; l += 13) {
            start = l;
            test_thread_mod(thread_count, test_strs[i], k, mod, congruence,
                            start, minimized_h);
          }
        }
      }
...
TEST_CASE("thread_wind function testing") {
  ...
  SECTION("Special Cases") {
    ...
    for (int i = 0; i < 10; i++) {
      for (int i = 0; i < 4; i += 2) {
        std::string str = test_strs[i].substr(start, 99);
        test_thread_wind(thread_count, str, k, large_wind_kmer_am, start,
                         minimized_h);
      }
    }
    ...
    for (int i = 0; i < 4; i++) {
      for (int i = 0; i < 4; i += 2) {
        for (int j = 4; j <= 64; j += 4) {
          ...
          for (int l = 0; l <= 96; l += 13) {
            start = l;
            test_thread_wind(thread_count, test_strs[i], k, large_wind_kmer_am,
                             start, minimized_h);
          }
        }
      }
...
TEST_CASE("thread_sync function testing") {
  ...
  SECTION("Special Cases") {
    ...
    for (int i = 0; i < 10; i++) {
      for (int i = 0; i < 4; i += 2) {
        std::string str = test_strs[i].substr(start, 99);
        test_thread_sync(thread_count, str, k, large_wind_kmer_am, start,
                         minimized_h);
      }
    }
    ...
    for (int i = 0; i < 4; i++) {
      for (int i = 0; i < 4; i += 2) {
        for (int j = 4; j <= 64; j += 4) {
          ...
          for (int l = 0; l <= 96; l += 13) {
            start = l;
            test_thread_sync(thread_count, test_strs[i], k, large_wind_kmer_am,
                             start, minimized_h);
          }
        }
      }
